...
# AIM OF SCRIPT - to run all combinations of hmms based on predictors specified in Section 2.5 of Methods of the manuscript and outputting models, 
# AIC tables and goodness of fit and autocorrelation plots


# The graphs will be stored in a "Plots" folder.
# First, we make sure we have it. Else, we create it.
out.path <- "./Plots/"
...
# artifact for zero step lengths of which there are only 2 values in full dataset
# (remember it's best to use the zeromass parameter)
table(dat$step == 0) # no zeros in this subset, but run the code below if there are 1+ zero values
...
# The exact same analysis from the previous section is run for Crozet data.
