...
# AIM OF SCRIPT - to load in data, run initial 3-state HMM from a range of sensible values for each state (directed flight, search and rest) 
# and determine if models consistently settle on same parameter values for each state.  
...
set.seed(1) # fixing the seed allows for comparable results, but you can remove this to make sampling random
...
# the order of these labels correspond to the one in our final results. You'll hae to alter it depending on your results.
...
# A proper way to deal with zeros is to use the zeromass parameters. As here, there are only 2 zero values this will not change results. 
...
#   ----------------
#         travel   search      rest
# mean 18.728783 5.758392 0.6221451
# sd    5.587003 6.474131 0.3246748
# 
# angle parameters:
#   -----------------
#                   travel       search        rest
# mean           0.002224857 -0.004025579  0.03487241
# concentration 52.969930473  1.307708454 51.26332577
...
# histograms of data, and then calculate all combinations of these
hist(dat$step) 
...
# it will generate a "decent" set of combinations of randomized bur realistic initial values for step and angle parameters
...
# specifying output lists of length 100
out.mod <- vector(mode = "list", length = nrow(all_vals))
out.df <- vector(mode = "list", length = nrow(all_vals))
...
ggplot(top.df, aes(Vals)) + geom_histogram(aes(fill = State))
...
# the values below from the "best" model are just for the subset (of 20 individuals)
# travel = 18.73
# search = 5.59
# rest = 0.36
...
head(out.all) # model number 79 was "best" for me, but will change each time as order will be randomized
plot(out.mod[[79]])
...
# This is exactly the same analysis as that ran with the South Georgia dataset.
...
#   ----------------
#         travel   search      rest
# mean 12.46129 3.950403 0.3400040
# sd    3.73476 4.442000 0.1949168
# 
# angle parameters:
#   -----------------
#                   travel       search        rest
# mean           0.003333838 -0.01591877  0.03050393
# concentration 47.152833229  1.16361860 39.00367103
...
# the values below are for the "best" model just for the subset (of 20 individuals)
# travel = 12.46
# search = 3.95
# rest = 0.34
...
head(out.all) # model number 8 was "best" for me, but will change each time as order will be randomized
plot(out.mod[[8]])
# look good although states are labelled in the incorrect order. 
