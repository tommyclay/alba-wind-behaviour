...
# (see more detailed descripted in README file an Appendix of the manuscript)
...
# for reduced dataset (i.e. n = 4), it will be a common occurrence that the sexes of randomized vs real datasets will match,
# but shouldn't happen often for full dataset.
# for dull dataset you want to iterate again so that all are different from real dataset - i.e. you want all same_df$Sexes_same to be "FALSE"
...
# specifying output lists of length n.iter
mod <- vector(mode = "list", length = n.iter)
gps.l <- vector(mode = "list", length = n.iter)
AIC.df <- vector(mode = "list", length = n.iter)
for (i in 1:n.iter) {
  print(i)
  gps.l[[i]] <- gps
  ...
  # the best approach is to use zeromass parameter instead
